# Ablation: filter threshold (tau) sweep for cross-query SE.
# Tests robustness of cross-query SE to the semantic-equivalence filter threshold.
# For each tau, re-applies the filter to existing raw perturbations and runs the
# full pipeline on a 1000-example subset per dataset.
#
# Reuses from base_output_dir: orig_retrieval, vllm_base, perturbations_raw_seed*,
#   retrieval_info.  Only the filter step is varied.
# New stages per tau: 2b (filter only), 3, 4, 5, 5b, 7, 8, 9.
# Output dir: cross_query_se/outputs/ablation_tau_{tau}/
...
# ─── Stage 2b: Re-apply filter at specified tau ───────────────────────────────

def stage2b_refilter(datasets, data, base_output_dir, output_dir, seeds, k_perturb,
                      tau, gpu_id, max_examples=-1):
    """Re-apply semantic equivalence filter at specified tau to existing raw perturbations."""
    logger.info(f"=== Stage 2b: Re-filtering perturbations at tau={tau} ===")

    filt = SemanticEquivalenceFilter(tau=tau, cosine_only=True, device=f"cuda:{gpu_id}")

    for ds in datasets:
        for seed in seeds:
            pert_out = os.path.join(output_dir, f"{ds}_perturbations_seed{seed}.jsonl")
            if os.path.exists(pert_out):
                logger.info(f"[{ds}] seed={seed} Filtered perturbations already exist at tau={tau}, skipping")
                continue

            raw_path = os.path.join(base_output_dir, f"{ds}_perturbations_raw_seed{seed}.jsonl")
            raw_records = _jsonl_load(raw_path)
            if max_examples > 0:
                raw_records = raw_records[:max_examples]
            ...
            logger.info(f"[{ds}] seed={seed} tau={tau} Saved {len(pert_records)} records, avg_filtered={avg_filt:.2f}")

    del filt
    gc.collect()
    torch.cuda.empty_cache()
    logger.info(f"Stage 2b (tau={tau}) done")
...
def stage7_threshold_and_ais(datasets, data, base_output_dir, output_dir, seeds, dev_size, top_k, top_k_dual,
                               embeddings_path, chunk_size, gpu_id, max_examples=-1):
    ...
    # For the tau sweep, the dev_size may exceed max_examples; cap it
    effective_dev_size = dev_size
    if max_examples > 0 and dev_size >= max_examples:
        effective_dev_size = max_examples // 2
        logger.info(f"Capping dev_size to {effective_dev_size} (max_examples={max_examples})")

    for ds in datasets:
        if ds in thresholds:
            logger.info(f"[{ds}] Thresholds already tuned, skipping")
        else:
            logger.info(f"[{ds}] Tuning thresholds on dev split [0:{effective_dev_size}]...")
            base_records = _jsonl_load(os.path.join(base_output_dir, f"{ds}_vllm_base.jsonl"))
            if max_examples > 0:
                base_records = base_records[:max_examples]
            dev_base = {r["question"]: r for r in base_records[:effective_dev_size]}

            avg_hcq_by_q = defaultdict(list)
            for seed in seeds:
                hcq_recs = _jsonl_load(os.path.join(output_dir, f"{ds}_hcq_seed{seed}.jsonl"))
                for r in hcq_recs[:effective_dev_size]:
                    avg_hcq_by_q[r["question"]].append(r["hcq_score"])

            majority_by_q = {}
            for seed in seeds:
                hcq_recs_dev = _jsonl_load(os.path.join(output_dir, f"{ds}_hcq_seed{seed}.jsonl"))
                for r in hcq_recs_dev[:effective_dev_size]:
    ...
    for ds in datasets:
        for seed in seeds:
            ...
            test_base = base_records[effective_dev_size:]
            test_hcq = hcq_records[effective_dev_size:]
...
def stage8_vllm_enhanced(datasets, data, base_output_dir, output_dir, seeds, dev_size, top_k, vllm_tp,
                          model=None, max_examples=-1):
...
def stage9_evaluate(datasets, data, base_output_dir, output_dir, seeds, dev_size, top_k, results_dir,
    ...
    for ds in datasets:
        ...
        for seed in seeds:
            pert_records = _jsonl_load(os.path.join(output_dir, f"{ds}_perturbations_seed{seed}.jsonl"))
            avg_n_filtered = float(np.mean([r["n_filtered"] for r in pert_records]))
            n_pass_filter_list.append(avg_n_filtered)
            ...
            seed_metrics.append({
                ...
                "avg_n_filtered": avg_n_filtered,
            })
            logger.info(
                f"[{ds}] seed={seed} tau={tau} EM={avg_em:.4f} F1={avg_f1:.4f} "
                f"AUROC={auroc:.4f} AvgFiltered={avg_n_filtered:.2f}"
            )
        ...
        all_results[ds] = {
            ...
            "avg_n_filtered_mean": float(np.mean(n_pass_filter_list)),
            "tau_low": tau_low,
            "tau_high": tau_high,
            "n_test": n_test,
            "retrieval_mode_pct": {
        ...
        logger.info(
            f"[{ds}] tau={tau} FINAL EM={all_results[ds]['em_mean']:.4f} "
            f"F1={all_results[ds]['f1_mean']:.4f} AUROC={all_results[ds]['auroc_mean']:.4f} "
            f"AvgFiltered={all_results[ds]['avg_n_filtered_mean']:.2f}"
        )

    summary_path = os.path.join(results_dir, f"ablation_tau_{tau}_results.json")
    ...
    print(f"\n=== ABLATION: TAU SWEEP tau={tau} ===")
    print(f"{'Dataset':<12} {'EM':>8} {'F1':>8} {'AUROC':>8} {'AvgFilt':>8}")
    print("-" * 50)
    for ds, r in all_results.items():
        print(
            f"{ds:<12} {r['em_mean']:>7.4f} "
            f"{r['f1_mean']:>7.4f} "
            f"{r['auroc_mean']:>7.4f} "
            f"{r['avg_n_filtered_mean']:>7.2f}"
        )
    return all_results
...
def main():
    ...
    parser.add_argument("--tau", type=float, default=0.85,
                        help="Semantic equivalence filter threshold to sweep")
    ...
    parser.add_argument("--results_dir", default="cross_query_se/results/ablation_tau_sweep")
    ...
    parser.add_argument("--max_examples", type=int, default=1000,
                        help="Limit dataset size (default=1000 for tau sweep)")
    parser.add_argument("--stages", nargs="+", type=int, default=[2, 3, 4, 5, 51, 7, 8, 9])
    args = parser.parse_args()

    # Derive output dir from tau value
    tau_str = f"{args.tau:.2f}".replace(".", "_")
    output_dir = f"cross_query_se/outputs/ablation_tau_{tau_str}"
    ...
    if 2 in args.stages:
        stage2b_refilter(args.datasets, data, args.base_output_dir, output_dir,
                          args.seeds, args.k_perturb, args.tau, args.gpu_id,
                          max_examples=args.max_examples)
    if 3 in args.stages:
        stage3_retrieval_perturbations(args.datasets, data, output_dir, EMBEDDINGS_PATH,
                                       args.chunk_size, args.top_k, args.gpu_id,
                                       query_batch_size=args.query_batch_size)
    if 4 in args.stages:
        stage4_vllm_perturbation_answers(args.datasets, data, output_dir, args.seeds,
                                          args.top_k, args.vllm_tp)
    if 5 in args.stages:
        stage5_hcq_computation(args.datasets, data, output_dir, args.seeds, args.gpu_id)
    if 51 in args.stages:
        stage5b_enrich_hcq(args.datasets, output_dir, args.seeds)
    if 7 in args.stages:
        stage7_threshold_and_ais(args.datasets, data, args.base_output_dir, output_dir,
                                  args.seeds, args.dev_size, args.top_k, args.top_k_dual,
                                  EMBEDDINGS_PATH, args.chunk_size, args.gpu_id,
                                  max_examples=args.max_examples)
    if 8 in args.stages:
        stage8_vllm_enhanced(args.datasets, data, args.base_output_dir, output_dir,
                              args.seeds, args.dev_size, args.top_k, args.vllm_tp,
                              max_examples=args.max_examples)
    if 9 in args.stages:
        stage9_evaluate(args.datasets, data, args.base_output_dir, output_dir,
                        args.seeds, args.dev_size, args.top_k, args.results_dir,
                        tau=args.tau, max_examples=args.max_examples)
