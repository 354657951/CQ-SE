# Ablation: w/o NLI filtering variant of cross-query SE.
# This variant corresponds to "CQ-SE w/o Re-retrieval" in the paper (Table 2),
# which has independent per-paraphrase retrieval but omits the NLI-based
# semantic clustering step. Instead of clustering answers by entailment,
# answer frequency is used directly for entropy computation.
# This isolates the contribution of NLI semantic clustering to the pipeline.
#
# Reuses from base_output_dir: orig_retrieval, vllm_base, perturbations_seed*,
#   retrieval_info (Stage 6 outputs).
# New stages run: 3 (copy), 4, 5, 5b, 7, 8, 9.
...
# ─── Stage 3 (ablation): copy original doc_ids to all perturbations ──────────

def stage3_copy_original_docs(datasets, data, base_output_dir, output_dir, seeds, top_k, max_examples=-1):
    """Replace per-perturbation retrieval with shared original-query docs for all perturbations."""
    logger.info("=== Stage 3 (ablation): Copy original docs to all perturbations ===")

    for ds in datasets:
        orig_ret_path = os.path.join(base_output_dir, f"{ds}_orig_retrieval.jsonl")
        orig_ret = {r["question"]: r for r in _jsonl_load(orig_ret_path)}

        for seed in seeds:
            out_path = os.path.join(output_dir, f"{ds}_pert_retrieval_seed{seed}.jsonl")
            if os.path.exists(out_path):
                logger.info(f"[{ds}] seed={seed} Stage 3 (copy) already done, skipping")
                continue

            pert_path = os.path.join(base_output_dir, f"{ds}_perturbations_seed{seed}.jsonl")
            pert_records = _jsonl_load(pert_path)
            if max_examples > 0:
                pert_records = pert_records[:max_examples]

            pert_ret_records = []
            for pr in pert_records:
                q = pr["question"]
                orig = orig_ret[q]
                orig_doc_ids = orig["doc_ids"][:top_k]
                orig_score = float(np.max(orig["scores"][:top_k])) if orig["scores"] else 0.0

                n_queries = 1 + len(pr["filtered_perturbations"])
                per_query_doc_ids = [sorted(orig_doc_ids) for _ in range(n_queries)]
                per_query_max_scores = [orig_score for _ in range(n_queries)]
            ...
            logger.info(f"[{ds}] seed={seed} Saved {len(pert_ret_records)} copied pert-retrieval records")

    logger.info("Stage 3 (copy) done")


# ─── Stage 4: vLLM pass 2 — per-perturbation greedy answers ─────────────────

def stage4_vllm_perturbation_answers(datasets, data, output_dir, seeds, top_k, vllm_tp, model=None, max_examples=-1):
    logger.info("=== Stage 4: vLLM pass 2 (per-perturbation greedy answers) ===")

    # Check if any seeds actually need generation; skip vLLM init if all outputs exist
    ...
    for ds in datasets:
        # Only collect doc IDs for seeds that need generation
        ...
        for seed in seeds_needed:
            out_path = os.path.join(output_dir, f"{ds}_vllm_pert_seed{seed}.jsonl")
            pert_ret_records = _jsonl_load(os.path.join(output_dir, f"{ds}_pert_retrieval_seed{seed}.jsonl"))
...
def stage7_threshold_and_ais(datasets, data, base_output_dir, output_dir, seeds, dev_size, top_k, top_k_dual,
    ...
    # AIS doc selection for test set
    device = f"cuda:{gpu_id}"
...
def stage9_evaluate(datasets, data, base_output_dir, output_dir, seeds, dev_size, top_k, results_dir):
        ...
        for seed in seeds:
            ...
            for br in test_base:
                ...
                if decision == "no_retrieval":
                    ...
                    retriever_calls = 1 + 1  # orig only (no per-pert retrieval in this ablation)
                    docs_retrieved = top_k
                else:
                    pred = enh_answers_by_q.get(q, majority_ans)
                    n_enhanced_ret += 1
                    retriever_calls = 1 + 1 + 1  # orig + 1 (shared) + pseudo
                    docs_retrieved = top_k
    ...
    summary_path = os.path.join(results_dir, "ablation_no_reretrieval_results.json")
    ...
    print("\n=== ABLATION: w/o RE-RETRIEVAL RESULTS ===")
...
def main():
    ...
    parser.add_argument("--output_dir", default="cross_query_se/outputs/ablation_no_reretrieval")
    parser.add_argument("--base_output_dir", default="cross_query_se/outputs/cross_query_se",
                        help="Base pipeline output dir to reuse Stage 1/2/6 outputs from")
    parser.add_argument("--results_dir", default="cross_query_se/results/ablation_no_reretrieval")
    ...
    parser.add_argument("--max_examples", type=int, default=-1, help="Limit dataset size (-1=all)")
    parser.add_argument("--stages", nargs="+", type=int, default=[3, 4, 5, 51, 7, 8, 9])
    ...
    if 3 in args.stages:
        stage3_copy_original_docs(args.datasets, data, args.base_output_dir, args.output_dir,
                                   args.seeds, args.top_k, max_examples=args.max_examples)
    if 4 in args.stages:
        stage4_vllm_perturbation_answers(args.datasets, data, args.output_dir, args.seeds,
                                          args.top_k, args.vllm_tp, max_examples=args.max_examples)
    if 5 in args.stages:
