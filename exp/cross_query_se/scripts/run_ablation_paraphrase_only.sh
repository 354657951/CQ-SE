#!/bin/bash
# Full-scale run: paraphrase-only ablation on all 5 datasets, seeds 0/1/2.
set -e
...
for DATASET in nq webqa triviaqa hotpotqa squad; do
  ...
  VLLM_WORKER_MULTIPROC_METHOD=spawn python cross_query_se/scripts/run_ablation_paraphrase_only.py \
    ...
    --stages 2 3 4 5 51 7
  echo "--- Done stages 2-7: $DATASET ---"
done

echo "=== Copying enhanced_answers from base pipeline (AIS docs are identical) ==="
ABLATION_DIR="cross_query_se/outputs/ablation_paraphrase_only"
for DATASET in nq webqa triviaqa hotpotqa squad; do
...
echo "=== Ablation Paraphrase-Only Complete ==="
