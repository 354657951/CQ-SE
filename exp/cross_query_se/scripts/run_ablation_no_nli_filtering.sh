#!/bin/bash
# Full-scale run: w/o NLI filtering ablation on all 5 datasets, seeds 0/1/2.
# Corresponds to "CQ-SE w/o Re-retrieval" in paper Table 2.
set -e
...
for DATASET in nq webqa triviaqa hotpotqa squad; do
  ...
  VLLM_WORKER_MULTIPROC_METHOD=spawn python cross_query_se/scripts/run_ablation_no_nli_filtering.py \
    --datasets $DATASET \
    --output_dir cross_query_se/outputs/ablation_no_nli_filtering \
    --base_output_dir "$BASE_DIR" \
    --results_dir cross_query_se/results/ablation_no_nli_filtering \
    --seeds 0 1 2 \
    --dev_size 500 \
    --top_k 5 \
    --top_k_dual 5 \
...
ABLATION_DIR="cross_query_se/outputs/ablation_no_nli_filtering"
for DATASET in nq webqa triviaqa hotpotqa squad; do
...
echo "=== Ablation w/o NLI Filtering Complete ==="
